...
export const signup = async (req, res, next) => {
  ...
  if (isAlreadyExist) return next(errorHandler(400, "User already exists"))
  ...
  const finalProfileImageUrl =
    profileImageUrl ||
    `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=random&color=fff&length=1`
...
export const updateUserProfile = async (req, res, next) => {
  try {
    ...
    const updated = await User.updateById(req.user.id, updates)
